#ifndef DYNAMIC_ARCHETYPES_ASSIGNABLE_HPP_INCLUDED_
#define DYNAMIC_ARCHETYPES_ASSIGNABLE_HPP_INCLUDED_
...
namespace dynamic_archetypes {
  ...
  /// @brief A dynamic archetype of Assignable
  struct assignable {
    
    assignable & operator=(assignable const & other);
    
  private:
    // Assignable does not mention the address-of operator, so we must
    // suppress it.
    struct Incomplete_Type;
    Incomplete_Type operator&() const;

    uint64_t identity_;
    friend class object_data<assignable>;
    assignable(uint64_t);
    ~assignable();
  };
  ...
  template<>
  bool object_data<assignable>::equivalent
  (assignable const &, assignable const &);
}
